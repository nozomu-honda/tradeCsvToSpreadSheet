...
現在の本番運用は、Webアプリを所有者本人だけが利用し、「自分（デプロイしたユーザー）」として実行する。`appsscript.json` では `webapp.access = MYSELF` / `webapp.executeAs = USER_DEPLOYING` を維持する。テスト用Web E2Eの一時manifest設定とは混同しない。
