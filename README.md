...
PRの最終CIは、現在のhead SHAとdevelop base SHAを記録したレビュー完了コメントを付けた後に `run-final-ci` ラベルで起動する。docs-onlyではActions自体を起動せず、backend GAS-onlyではGAS Testsだけ、UI・Web・manifest・deployment・E2E関連ではGAS Tests成功後にWeb E2Eを実行する。同じhead/baseの組に対する成功Check Runだけを再利用し、旧ラベルの `run-gas-tests` / `gas-web-e2e` は最終CIの起動には使わない。詳細は [`docs/gas-ci.md`](docs/gas-ci.md) を参照する。
