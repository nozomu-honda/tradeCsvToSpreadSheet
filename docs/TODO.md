...
- PR #96マージ後のAuthenticated dry-run 29436843157は `deployment-target-verification` で失敗したが、抽象エラーだけで原因を分類できなかった。更新前strict検証と更新後comparison snapshotを分離した安全なreasonをログとSummaryへ追加してから再確認する。
...
3. 安全なreason分類を反映後、必要に応じて `deploy-production-dry-run` でAuthenticated dry-runを行い、Web App URLとdeployment ID、更新前strict検証、Tracked / Untracked境界を確認する。失敗時はActionsログとStep Summaryのreasonから設定またはdeployment状態を確認する。
...
- 更新後comparison snapshotから `WEB_APP`消失、件数、entry point type、URL、access、executeAsの変更が更新後専用reasonへ分類される。
- Web App検証reasonやログにURL fingerprint、snapshot、credential JSON、Apps Script APIレスポンス全文、Google API内部エラー本文を含めない。
