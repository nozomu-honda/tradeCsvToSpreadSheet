...
- 楽天米国株の実取込結果の最終確認。
- 楽天専用の出力処理本体は `楽天日本株` / `楽天米国株` の初期専用シート対応まで完了。楽天投信、配当金、金銭残高のDrive最終見た目対応は未実装。
- 楽天専用ロールバックUI分離は未実装。

## 直近の優先順位

1. 楽天投信、配当金、金銭残高の専用出力を、`groupRakutenOutputRecords_()` / `buildRakutenOutputSheetsFromBaseRecords_()` 配下で段階的に実装する。
