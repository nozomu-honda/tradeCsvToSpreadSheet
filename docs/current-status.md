...
- 楽天専用の出力処理本体は `楽天日本株` / `楽天米国株` / `楽天投資信託` / `金銭残高（円）` / `金銭残高（ドル）` の初期専用シート対応まで完了。配当金・分配金の完全な専用出力は未実装。
- 楽天専用ロールバックUI分離は未実装。

## 直近の優先順位

1. 配当金・分配金の専用出力を、`groupRakutenOutputRecords_()` / `buildRakutenOutputSheetsFromBaseRecords_()` 配下で段階的に実装する。
