...
- 利用者が編集するデータシートは 1 枚のみ作成する
- シート名は `取引履歴_一次受け枠`
- 元のヘッダー末尾に追加7カラムを付ける
- 証券会社、`sourceType`、楽天固有の元データ補完情報は、利用者から隠した内部メタデータシートへ保存する
- 本処理では内部メタデータを優先し、一次受け枠の正規化済み行を野村形式として再判定しない
...
## 14. 現状コードとの差分メモ（引き継ぎ用）

### 14.1 すでにコード側へ反映済みの主なもの

- `外債` 用出力名 `OUTPUT_FOREIGN_BOND`
- `外債` の非表示列設定
- 外貨買付系簿価は、`受渡金額 × レート - 手数料の消費税額`
- `入金（分配金）` の簿価・残高不加算
- `test DB` の固定出力設定
- 6シート前提のテスト登録
...
- Web UI のタイトル / ボタン文言がまだ 5シートまたは4シート表記
- 実行結果表示に `外債件数` が未表示
- タブ順を明示的に並べ替える helper が未接続
- `web.gs` に `runStagingSheetFromWebApp` が重複定義のまま残っている可能性がある

### 14.3 次チャットでまず確認するファイル
...
- 次チャットへ引き継ぐときは、`spec.md` と `trade-rules.md` と `TODO.md` をセットで渡す
